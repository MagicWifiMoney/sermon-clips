...
1.  **Run an agent** – Call `/agent/[agent_id]/run` with `video_urls` (supports YouTube URLs and signed URLs).
2.  **Track progress** – Poll `/agent_run/[run_id]` or subscribe to a webhook.
3.  **Download outputs** – The outputs are included in the agent run status response.
...
When a webhook secret is configured for your account, Mosaic includes an `X-Mosaic-Signature` header for verification. If configured, always validate this signature before processing webhook payloads. See the [Webhooks](/api/webhooks) documentation for details.
